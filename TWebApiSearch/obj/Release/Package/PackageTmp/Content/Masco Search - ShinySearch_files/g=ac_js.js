...
function googleSuggest() {
    if (keyWords.length > 0) {
        var url = 'http://localhost:62707/api/v1/TService/Autocomplete2?text=' + encodeURIComponent($("#search_field_v2").val());//'https://www.google.com/complete/search?client=hp&q=' + encodeURIComponent(keyWords);
        return jsRemote(url);
    }
}
...
function AutoSuggest(elem, div, suggestions) {
    var me = this;
    this.elem = elem;
    ...
    this.useSuggestion = function() {
        if (this.highlighted > -1) {
            ...
            if (val != "null" && val != "undefined") {
                ...
                this.inputText = this.elem.value;
            }
            this.hideDiv();
    ...
    this.setText = function() {
        ...
        if (val != "null" && val != "undefined") {
            ...
            this.elem.value = this.inputText;
        }
    }
    this.positionDiv = function() {
        var jElem = $(this.elem);
        var elem_offset = jElem.offset();
        elem_offset.top += jElem.outerHeight() + 1;
        if (this.elem.id == "search_field_wide") {
            elem_offset.top += 7;
        }
        elem_offset.width = jElem.outerWidth();
        window.setTimeout(function() {
            $(this.div).offset(elem_offset).outerWidth(elem_offset.width);
        }, 0)
    }
    ;
